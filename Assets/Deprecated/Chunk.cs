...
public class Chunk : MonoBehaviour
{
        ...
        for (int x = 0; x < chunkSize; x++)
        {
            for (int y = 0; y < chunkSize; y++)
            {
                for (int z = 0; z < chunkSize; z++)
                {
                    ...
                    Voxel.VoxelType type = Voxel.VoxelType.Air; //DetermineVoxelType(worldPos.x, worldPos.y, worldPos.z);
    ...
    {
        //float noiseValue = Mathf.PerlinNoise(x, y) * Random.value; 
        ////float noiseValue = Random.value;
        //float threshold = 0.15f; // The threshold for determining solid/air
    ...
    {
        bool cannotProcess = voxels == null || x < 0 || x >= voxels.GetLength(0) || y < 0 || y >= voxels.GetLength(1) || z < 0 || z >= voxels.GetLength(2);
        if (cannotProcess)
